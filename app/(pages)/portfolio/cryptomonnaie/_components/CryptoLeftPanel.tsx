...
import CryptoPieChart from '@/components/Charts/CryptoPage/CryptoPieChart/CryptoPieChart';
      ...
      <Grid size={12} sx={{ height: 'calc(25% - 8px)' }}>
        <CustomCard title="Valeur du wallet" sx={{ height: '100%' }}>
          {loading ? (
            ...
            <>
              <Typography sx={{ fontSize: 36, fontWeight: 'bold', color: '#7F00FF', letterSpacing: 1, fontFamily: "'Roboto', sans-serif" }}>
      ...
      <Grid size={12} sx={{ height: 'calc(75% - 8px)' }}>
        <CustomCard title="Répartition du wallet" sx={{ height: '100%' }}>
          <Box sx={{ height: '100%', mt: 4 }}>
            <CryptoPieChart data={cryptoData.map((crypto) => ({ label: crypto.ticker, value: crypto.price * crypto.amount }))} />
          </Box>
        </CustomCard>
      </Grid>
    </Grid>
